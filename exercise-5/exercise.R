...
words <- c('apple', 'banana', 'avocado', 'mango', 'baby', 'cse', 'flying', 'z')
...
a.f.words <- words[substr(words, 1, 1) >= 'a' & substr(words, 1, 1) <= 'f']
print(a.f.words)
...
WordBin <- function (words, letter.1, letter.2){
	return(words[substr(words, 1, 1) >= letter.1 & substr(words, 1, 1) <= letter.2])
}
